...
class TradeCostToolTests(unittest.TestCase):
    def test_prefers_anon_rpc_before_a_stale_service_client(self):
        async def exercise():
            ...
            with patch("app.agent.tools.get_service_client", return_value=service_client) as get_service_client, patch(
                "app.agent.tools.get_supabase", return_value=_SuccessfulClient()
            ):
                result = await get_trade_costs("CN", "UZ", "610910")
            return result, service_client, get_service_client

        result, service_client, get_service_client = asyncio.run(exercise())
