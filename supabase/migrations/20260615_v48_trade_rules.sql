...
SELECT * FROM (VALUES
    ...
    ('RU', 'EAEU', 'Yevrosiyo Iqtisodiy Ittifoqi doirasida erkin savdo', 'EAEU Free Trade Regime (Observer)', 100,
     ...
     '2015-01-01'::date, null::date, 'EAEU: Russia, Kazakhstan, Belarus, Kyrgyzstan, Armenia. Uzbekistan joined as observer in 2020. Zero duty on most industrial goods.'),
    ('KZ', 'EAEU', 'Yevrosiyo Iqtisodiy Ittifoqi doirasida erkin savdo', 'EAEU Free Trade Regime (Observer)', 100,
     'Goods must originate from EAEU member state. Certificate of Origin CT-1 required. At least 50% local content.',
     '2015-01-01'::date, null::date, 'Same EAEU regime as Russia. Kazakhstan is also the main transit country for CN→UZ rail.'),
    ('BY', 'EAEU', 'Yevrosiyo Iqtisodiy Ittifoqi doirasida erkin savdo', 'EAEU Free Trade Regime (Observer)', 100,
     'Goods must originate from EAEU member state. Certificate of Origin CT-1 required. At least 50% local content.',
     '2015-01-01'::date, null::date, 'EAEU member. Industrial machinery and food products from Belarus enter duty-free.'),
    ('KG', 'EAEU', 'Yevrosiyo Iqtisodiy Ittifoqi doirasida erkin savdo', 'EAEU Free Trade Regime (Observer)', 100,
     'Goods must originate from EAEU member state. Certificate of Origin CT-1 required. At least 50% local content.',
     '2015-01-01'::date, null::date, 'EAEU member. Kyrgyzstan is also a transit country for CN→UZ road freight.'),
    ('AM', 'EAEU', 'Yevrosiyo Iqtisodiy Ittifoqi doirasida erkin savdo', 'EAEU Free Trade Regime (Observer)', 100,
     'Goods must originate from EAEU member state. Certificate of Origin CT-1 required. At least 50% local content.',
     '2015-01-01'::date, null::date, 'EAEU member. Limited trade volume but full duty preference.'),
    ...
    ('RU', 'CIS_FTA', 'MDH Erkin Savdo Hududi', 'CIS Free Trade Area', 100,
     ...
     '2012-01-01'::date, null::date, 'CIS FTA: Uzbekistan, Russia, Kazakhstan, Belarus, Kyrgyzstan, Armenia, Moldova, Tajikistan. Overlaps with EAEU for most members.'),
    ('KZ', 'CIS_FTA', 'MDH Erkin Savdo Hududi', 'CIS Free Trade Area', 100,
     'Certificate of Origin CT-1 required. Goods must have at least 50% local content from CIS member.',
     '2012-01-01'::date, null::date, 'Same as CIS FTA.'),
    ...
    ('TM', 'FTA', 'O''zbekiston-Turkmaniston Erkin Savdo Kelishuvi', 'Uzbekistan-Turkmenistan Free Trade Agreement', 100,
     ...
     '2025-02-25'::date, null::date, 'Signed July 2024, entered into force Feb 2025. Zero duty on most goods except certain categories.'),
    ('TR', 'PTA', 'O''zbekiston-Turkiya Preferensial Savdo Kelishuvi', 'Uzbekistan-Turkey Preferential Trade Agreement', 50,
     ...
     '2023-01-01'::date, null::date, 'Signed 2023. Covers ~200 product categories with reduced duties (average 50% reduction). Currently being expanded to full FTA.')
) AS seed_data
...
WHERE NOT EXISTS (SELECT 1 FROM certification_requirements WHERE hs_chapter = seed_data.column1 AND (cert_type = seed_data.column3 OR (cert_type IS NULL AND seed_data.column3 IS NULL)));
